...
def variance_scaling(scale, mode, distribution,
  ...
  def _compute_fans(shape, in_axis=1, out_axis=0):
    # 卷积核特有:shape = [out_ch, in_ch, kernal_h, kernal_w]
    # receptive_field_size=计算卷积核的覆盖面积
  ...
  def init(shape, dtype=dtype, device=device):
    ...
    # 假设是fan_in，假设W维度为[2,3]，如下：
    # W = [ [ 0.5,  -0.2,  0.4],    ← y₁ 的三个权重
    #       [ 0.3,   0.6, -0.1] ]   ← y₂ 的三个权重
    # x@WT > y1 = 0.5*x1 - 0.2*x2 + 0.4*x3 | y2 = 0.3*x1 + 0.6*x2 - 0.1*x3
    # 选择fan_in即主要考虑前向传播：Var(y) = fan_in * Var(w) * Var(x) 即y的总方差= 参数数量 * 各参数方差 * x各维度方差 | 主要目标是Var(y) = Var(x) 即方差保持
    # Var(y) = fan_in * scale/fan_in * Var(x) = scale * Var(x)
      return torch.randn(*shape, dtype=dtype, device=device) * np.sqrt(variance)
    elif distribution == "uniform":
    # torch.rand的区间为[0, 1]，方差是1/12 | *2-1的区间为[-1,1]，方差是1/3
    # 因此后续缩放为 3 * scale / fan_in才行
...
def get_timestep_embedding(timesteps, embedding_dim, max_positions=10000):
  ...
  # 通过遍历half_dim【0,1,2,...,63】，分别相乘emb，得到log(10000) * (-【1,2,3,...,63】/63) => log( 10000 ^ (-【1,2,3,...,63】/63) ) 
  # => log( 1 / ( 10000^ 【1,2,3,...,63】/63) )
  # exp => 1 / ( 10000^ 【1,2,3,...,63】/63)，就是常规正余弦位置编码的频率
  emb = torch.exp(torch.arange(half_dim, dtype=torch.float32, device=timesteps.device) * -emb)
  # 得到拓展Batch_size维度的emb
...
def contract_inner(x, y):
  """tensordot(x, y, 1)."""
  # 自定义设置矩阵内积的算法，目标是让X矩阵最后维度@W矩阵首个维度，即[A,B] @ [B,C] = [A,C]
  x_chars = list(string.ascii_lowercase[:len(x.shape)])     # 从ASCII码表按照顺序获取[A,B,C,D]
  y_chars = list(string.ascii_lowercase[len(x.shape):len(y.shape) + len(x.shape)])  # 从ASCII码表按照顺序获取[E,F]
  y_chars[0] = x_chars[-1]  # [E,F] > [D,F]
  out_chars = x_chars[:-1] + y_chars[1:]  # [A,B,C] + [F] = [A,B,C,F]
...
class NIN(nn.Module):
  def __init__(self, in_dim, num_units, init_scale=0.1):
    super().__init__()
    # init_scale=0.1，即初始化矩阵W权重的目标scale=0.1
    # (in_dim, num_units)是矩阵W的维度，num_units=out_dim
  ...
  def forward(self, x):
    ...
    y = contract_inner(x, self.W) + self.b  # 简单理解就是矩阵乘法+bias
    return y.permute(0, 3, 1, 2)  # [B,H,W,C] > [B,C,H,W]
