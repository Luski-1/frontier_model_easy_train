...
class EnhancedUNet(nn.Module):
    def __init__(self, in_ch=3, base_ch=128, time_emb_dim=512, num_res_blocks=2, group_num=16):
        ...
        self.down2 = DownBlock(base_ch, base_ch * 2, time_emb_dim, num_res_blocks, downsample=True, use_attention=True)         # 仅在16*16分辨率的情况下进行Attention
        self.down3 = DownBlock(base_ch * 2, base_ch * 4, time_emb_dim, num_res_blocks, downsample=True, use_attention=False)
        self.down4 = DownBlock(base_ch * 4, base_ch * 4, time_emb_dim, num_res_blocks, downsample=False, use_attention=False)

        # Middle block
        self.mid = MidBlock(base_ch * 4, time_emb_dim, num_res_blocks * 2)  # 开启Attention

        # decoder层
        self.up4 = UpBlock(base_ch * 4, base_ch * 4, skip_chs=[base_ch * 4] * num_res_blocks,   time_emb_dim=time_emb_dim, num_blocks=num_res_blocks, upsample=True, use_attention=False)
        self.up3 = UpBlock(base_ch * 4, base_ch * 2, skip_chs=[base_ch * 4] * num_res_blocks,   time_emb_dim=time_emb_dim, num_blocks=num_res_blocks, upsample=True, use_attention=False)
        self.up2 = UpBlock(base_ch * 2, base_ch,     skip_chs=[base_ch * 2] * num_res_blocks,   time_emb_dim=time_emb_dim, num_blocks=num_res_blocks, upsample=True, use_attention=True)
        self.up1 = UpBlock(base_ch,     base_ch,     skip_chs=[base_ch] * (num_res_blocks + 1), time_emb_dim=time_emb_dim, num_blocks=num_res_blocks + 1, upsample=False, use_attention=False)
