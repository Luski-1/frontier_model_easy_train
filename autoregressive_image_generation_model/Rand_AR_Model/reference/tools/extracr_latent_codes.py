...
def main(args):
    ...
    if is_main_process():
        ...
        for i in range(dataset.nb_classes):
            os.makedirs(
                f'{args.code_path}/{args.dataset}-{args.tokenizer_name}-{args.aug_mode}-{args.image_size}_codes/{i}',
                exist_ok=True)
    ...
    for x, y, index in data_loader:
        ...
        for i in range(x.shape[0]):
            np.save(
                f'{args.code_path}/{args.dataset}-{args.tokenizer_name}-{args.aug_mode}-{args.image_size}_codes/{y[i]}/{index[i]}.npy',
                x[i])
