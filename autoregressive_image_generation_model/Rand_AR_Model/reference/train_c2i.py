...
def main(args):
    ...
    accelerator_config = ProjectConfiguration(project_dir=experiment_dir)  # 设置实验目录，主要用于保存训练期间各种文件
    ddp_kwargs = DistributedDataParallelKwargs(find_unused_parameters=True)  # 设置DDP默认参数，让DDP允许部分参数不参与forward过程
    ...
    # args.data_path = /tmp/imagenet-llamagen-adm-256_codes
    # 获取dataset
    ...
    logger.info(
        "Datasets contains {} samples for {} batches".format(len(dataset), len(dataset) // config.global_batch_size))
    ...
    while train_steps < total_iters:
        ...
        image_tokens = x.reshape(x.shape[0], -1) # 如果是原始图像，则维度是B, H*W*C；如果是VQVAE处理过的图像，则维度是B, 
        cond = y.reshape(-1) # B
        #
        with accelerator.accumulate(model):
            ...
            for p in model.parameters():
                if p.grad is not None:  # 【新增安全监测】
                    grad_norm += p.grad.data.norm(2).item()
            # 当梯度大于10并且训练步数超出100时，不执行参数更新，保存模型训练稳定
            if grad_norm < config.optimizer.skip_grad_norm or train_steps < config.optimizer.skip_grad_iter:
                optimizer.step()    # 手动调用，如果该micro step没到达global step，accelerator会拦截调用

            optimizer.zero_grad()   # 手动调用，如果该micro step没到达global step，accelerator会拦截调用
            lr_scheduler.step() # 手动调用，如果该micro step没到达global step，accelerator会拦截调用
            running_loss += (
                    accelerator.gather(
                        loss.repeat(per_gpu_batch_size)).mean().item() / config.accelerator.gradient_accumulation_steps
            )
            running_grad_norm += (
        ...
        if accelerator.sync_gradients:
            ...
            if train_steps % args.log_every == 0 and accelerator.is_main_process:
                ...
                logger.info(
                    f"Step {train_steps:08d} | Loss {average_loss:.4f} | Time {average_time:.4f}s | Grad Norm {average_grad_norm:.4f} | LR {lr_scheduler.get_last_lr()[0]:.5f}")
                running_loss = 0
                running_grad_norm = 0
