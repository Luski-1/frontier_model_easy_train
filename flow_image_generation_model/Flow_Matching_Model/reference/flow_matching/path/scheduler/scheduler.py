...
class CondOTScheduler(ConvexScheduler):
    # 最优传输的概率密度路径，即直线传输或者称作rectified flow
    # 即(1 - t) * ε + t * x1，其中ε=x0=随机噪声N~(0, I)
    """CondOT Scheduler."""
    ...
    def __call__(self, t: Tensor) -> SchedulerOutput:
        return SchedulerOutput(
            alpha_t=t,
            sigma_t=1 - t,
            d_alpha_t=torch.ones_like(t),   # 对应的求导
...
class PolynomialConvexScheduler(ConvexScheduler):
    # 离散：多元幂的概率密度路径
    # 即 (1 - t^n) *ε + t^n * x1，其中ε=x0=随机噪声N~(0, I)
    # 当n=1时，就是最优传输的概率密度路径
    ...
    def __call__(self, t: Tensor) -> SchedulerOutput:
        return SchedulerOutput(
            alpha_t=t**self.n,
            sigma_t=1 - t**self.n,
            d_alpha_t=self.n * (t ** (self.n - 1)), # 对应的求导
