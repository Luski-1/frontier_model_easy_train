...
class ODESolver(Solver):
    ...
    def sample(
        ...
        **model_extras,
        ...
        with torch.set_grad_enabled(enable_grad):   # 停止梯度
            # torchdiffeq.odeint来自Neural-ODE官方项目的源码
            sol = odeint(
                ode_func,
                x_init,
                time_grid,
            ...
            # 可以根据下方精简实现理解大概逻辑
            # def odeint(x_init, time_grid, ode_opts, ode_func):                                   
            #    h_max = ode_opts["step_size"]
            #    z = x_init
            #    for index in range(len(time_grid) - 1):
            #      n_steps = math.ceil((abs(time_grid[index + 1] - time_grid[index])/h_max).max().item())      # 计算当前时间区间要拆分多少小步

            #      h = (time_grid[index + 1] - time_grid[index])/n_steps           # 计算步长
            #      t = time_grid[index]

            #       for i_step in range(n_steps):
            #           z = z + h * ode_func(z, t)         # 欧拉法更新z_t，其中ode_func就是模型预测的向量场
            #           t = t + h                          # 欧拉法更新t
            #    return z
