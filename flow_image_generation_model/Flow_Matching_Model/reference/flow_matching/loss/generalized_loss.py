...
class MixturePathGeneralizedKL(_Loss):
    ...
    def forward(self, logits: Tensor, x_1: Tensor, x_t: Tensor, t: Tensor) -> Tensor:
        ...
        log_p_1t = torch.log_softmax(logits, dim=-1)                # softmax归一化获得概率，log获得对数概率    [bs, 2, 128]
        log_p_1t_x1 = torch.gather(log_p_1t, dim=-1, index=x_1.unsqueeze(-1))   # 根据x1 [bs, 2]的对应token，从log_p_1t中拿出对应token的对数概率
        log_p_1t_x1 = log_p_1t_x1.view(*x_1_shape)  # [bs, 2]

        # extract x_t value of p_{1|t}(x|x_t).
        p_1t = torch.exp(log_p_1t)  # 概率 [bs, 2, 128]
        p_1t_xt = torch.gather(p_1t, dim=-1, index=x_t.unsqueeze(-1))   # 根据x1 [bs, 2]的对应token，从log_p_1t中拿出对应token的概率
        p_1t_xt = p_1t_xt.view(*x_1_shape)  # [bs, 2]

        scheduler_output = self.path.scheduler(t)   # 从PolynomialConvexScheduler获得scheduler_output

        jump_coefficient = (                        # 该损失函数和MDLM/LLADA一致，建议留到离散文本模型相关课程时再学习
