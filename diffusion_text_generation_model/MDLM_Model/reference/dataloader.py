...
def _group_texts(examples, block_size, bos, eos):
  ...
  concatenated_examples = list(itertools.chain(* examples['input_ids']))  # 拆包获得[[xxx, xxx]...]的token ids列表，并且拼接为[xxx,xxx,....]
  ...
  total_length = (total_length // new_block_size) * new_block_size  # 计算多少个block
  ...
  for i in range(0, total_length, new_block_size):
    _values.append(
      ...
      + [eos])
    _attn_masks.append(torch.ones(block_size))  # 设置全为1的mask
...
def get_dataset(
  ...
  if utils.fsspec_exists(_path):                                # 如果有缓存，直接读取并返回
  ...
  if dataset_name == 'wikitext103':
    ...
    dataset = datasets.load_dataset(            # 默认该路径
      'openwebtext',
      split='train[:-100000]',
      cache_dir=cache_dir,
      streaming=streaming)                      # False
  elif dataset_name == 'openwebtext-valid':     # 默认该路径
    dataset = datasets.load_dataset(
      'openwebtext',
      split='train[-100000:]',
      cache_dir=cache_dir,
  ...
  def preprocess_and_tokenize(example):
    ...
    if detokenizer is not None:
      text = _apply_detokenizer(detokenizer)(text)  # 执行字符串替换
    ...
    if wrap:
      ...
      tokens = {'input_ids':
                [t + [EOS] for t in tokens['input_ids']]} # 增加EOS token id
  ...
  if dataset_name == 'ptb':
    ...
    tokenized_dataset = tokenized_dataset.remove_columns( # 删除指定的文本列
      'text')
  ...
  group_texts = functools.partial(                              # 返回packing后的input_ids和attention_mask
  ...
  if streaming:   # 不走该分支
    ...
    chunked_dataset = tokenized_dataset.map(
      ...
      load_from_cache_file=True,
      desc='Grouping')
    chunked_dataset.save_to_disk(_path)   # 保存到磁盘
...
def get_dataloaders(config, tokenizer, skip_train=False,
  ...
  if skip_train:
    ...
    train_set = get_dataset(                          # 获取已经添加BOS和EOS的packing化的训练数据，包含input_ids和attention_mask
      config.data.train,
      tokenizer,
      mode='train',
      wrap=config.data.wrap,                          # True
      cache_dir=config.data.cache_dir,                # /share/kuleshov/ssahoo/textdiffusion/data
      block_size=config.model.length)                 # 1024
  ...
  if skip_train:
    ...
    train_loader = torch.utils.data.DataLoader(       # 获取train的dataloader
      train_set,
    ...
    train_loader.tokenizer = tokenizer                # 设置tokenizer
