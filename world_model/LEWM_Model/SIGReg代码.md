...
在实际应用中，我们通常不知道数据的真实分布，只能通过有限样本进行估计。**经验特征函数**就是用样本数据估计特征函数的方法。

**定义**：给定样本 $x_1, x_2, \ldots, x_n$ 来自某个未知分布，经验特征函数定义为：
$$\hat{\varphi}(t) = \frac{1}{n} \sum_{j=1}^{n} e^{itx_j}$$

**几何解释**：这实际上是 n 个复数 $e^{itx_1}, e^{itx_2}, \ldots, e^{itx_n}$ 求平均。当频率 t = 0 时， $e^{i \cdot 0 \cdot x_j} = 1$，所以 $\hat{\varphi}(0) = 1$。
...
**Epps-Pulley 检验**是一种基于特征函数的拟合优度检验（Goodness-of-Fit Test），用于检验样本数据是否来自某个特定分布。

**核心思想**：如果我们想检验数据是否服从高斯分布，只需要比较：
1. 数据对应的经验特征函数
2. 高斯分布的理论特征函数
...
**为什么这个公式有效？** 公式中的绝对值平方 $| \hat{\varphi}(t) - \varphi(t) |^2$ 度量了经验特征函数与理论特征函数在频率 t 处的差异。权重函数 $\omega(t)$ 确保我们在高频区域给予较小的权重，因为高频区域通常估计不稳定。只要积分值小就代表两者特征函数相近，即两者分布相近。

**积分的计算**：在实际实现中，我们不能直接计算无穷区间的积分，而是通过**数值积分**来近似：
$$T \approx n \cdot \sum_{k=1}^{K} \left| \hat{\varphi}(t_k) - \varphi(t_k) \right|^2 \cdot w_k$$

其中 $t_k$ 是采样的频率点，$w_k$ 是对应的频率权重。这就是代码中 `knots=17` 的意义——我们用 17 个采样频率来近似这个积分。
...
**同向高斯分布**是高斯分布的一种特殊形式，其特点是在所有方向上具有相同的方差。

**定义**：d 维同向高斯分布记作 $\mathcal{N}(\mathbf{0}, \mathbf{I}_d)$，其概率密度函数为：
...
- 生成从 0 到 3 的等间距数列
- 当 `knots=17` 时，生成 17 个频率点：$t = [0, 0.1875, 0.375, 0.5625, \ldots, 3.0]$
...
**数学含义**：对每个投影值 x 和每个采样频率 $t_k$，计算 $x \cdot t_k$。
...
**第 1 部分**：`x_t.cos()`
- 对 $x\_t$ 中的每个元素计算余弦： $\cos(x_t)$，形状： $(T, B, 1024, 17)$

**第 2 部分**：`x_t.sin()`
- 对 x_t 中的每个元素计算正弦： $\sin(x_t)$，形状： (T, B, 1024, 17)

**第 3 部分**：`mean(-3)`
- 对第 3 个维度（从后数，即 B 维度）求平均，形状从 $(T, B, 1024, 17)$ 变为 $(T, 1024, 17)$
- **数学含义**：这就是计算 **经验** 特征函数的实部和虚部！
...
**第 5 部分**：`.square()` 复数模的定义
- 计算差的平方： $[\text{Real}(\hat{\varphi}(t_k)) - \varphi(t_k)]^2$

**第 6 部分**：`+ x_t.sin().mean(-3).square()` 复数模的定义
- 这是虚部的平方： $[\text{Imag}(\hat{\varphi}(t_k)) - 0]^2 = \left[ \frac{1}{B}\sum_{t=1}^{B} \sin(x_t \cdot t_k) \right]^2$
...
```
x_t (简化 T=1, B=2, num_proj=1, knots=1):
数据 0: [0.000] # x = 2.118, t = 0.0
数据 1: [0.000] # x = 1.412, t = 0.0

self.phi (t=0.0):[1.000]

x_t.cos():
数据 0: [cos(0.000)] = [1.000]
数据 1: [cos(0.000)] = [1.000]

x_t.sin():
数据 0: [sin(0.000)] = [0.000]
数据 1: [sin(0.000)] = [0.000]

x_t.cos().mean(-3): # 对B维度求平均
= (1.000 + 1.000) / 2 = 1.000
...
err:
= (1.000 - 1.000)² + (0.000  - 0.000)²   # 标准正态分布在t=0时的特征为1(1+0i)
= 0
```
...
**代码含义**：对所有采样频率加权求和，得到 Epps-Pulley 统计量。
...
初始化 (仅在 __init__ 中执行一次):
 self.t: (knots,) = (17,) = 频率t数量
 self.phi: window: (17,) = exp(-t²/2) = 频率t权重 或 标准正态分布的特征函数
...
err: (T, num_proj, knots) [Epps-Pulley 误差 x_t - self.phi]

err @ self.weights: (T, num_proj) [频率t的加权求和，以及离散积分]
...
### 提问AI1：频率权重项与 $\varphi(t)$ 一致是否没问题？能否替换为其他权重函数？
