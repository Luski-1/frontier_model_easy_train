...
def get_img_preprocessor(source: str, target: str, img_size: int = 224):
    imagenet_stats = dt.dataset_stats.ImageNet  # dict(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    to_image = dt.transforms.ToImage(**imagenet_stats, source=source, target=target)
    resize = dt.transforms.Resize(img_size, source=source, target=target)
    return dt.transforms.Compose(to_image, resize) # tv_tensors.Image类型 > 自动缩放uint8(0-255) → float32(0-1) > ImageNet归一化 > resize 224
