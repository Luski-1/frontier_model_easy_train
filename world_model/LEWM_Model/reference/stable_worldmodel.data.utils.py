...
def get_cache_dir(
    ...
    if override_root is None:
        base = os.getenv('STABLEWM_HOME', str(DEFAULT_CACHE_DIR))   # DEFAULT_CACHE_DIR: os.path.expanduser('~/.stable_worldmodel')
...
def load_dataset(
    ...
    **kwargs,
    ...
    # 获取Path(cache_dir/datasets)对象，如果cache_dir为None则获取STABLEWM_HOME环境变量的Path对象，STABLEWM_HOME环境变量默认为os.path.expanduser('~/.stable_worldmodel')
    datasets_dir = get_cache_dir(cache_dir, sub_folder='datasets')
    ensure_dir_exists(datasets_dir)
    # 拼接name和datasets_dir获得完整路径
