...
class CondMSFBlock(nn.Module):
    def __init__(self, in_planes, features, num_classes, normalizer):
        """
        in_planes: 指输入的x的通道数，x是包含多层U-Net左侧输出的列表
        features: 最终输出的通道数
        num_classes: 噪声强度的类别数量
        normalizer: 条件式归一化ConditionalInstanceNorm2dPlus

        把x中不同输入，调整为相同通道数以及尺寸（上采样），并且相加为单个输出
        """
        super().__init__()
...
class CondRefineBlock(nn.Module):
    def __init__(self, in_planes, features, num_classes, normalizer, act=nn.ReLU(), start=False, end=False):
        ...
        self.n_blocks = n_blocks = len(in_planes)   # x输入的数量=block总数

        self.adapt_convs = nn.ModuleList()
        for i in range(n_blocks):
            self.adapt_convs.append(
                # 输入通道数，多少个block，每个block有多少个stage，噪声强度的类别数量，归一化，激活函数
                CondRCUBlock(in_planes[i], 2, 2, num_classes, normalizer, act)  # 简单而言就是ConditionalResidualBlock条件式归一化+卷积的残差Block
            )

        self.output_convs = CondRCUBlock(features, 3 if end else 1, 2, num_classes, normalizer, act)
        # 开始层的x仅有一个输入，因此不需要调整维度
        if not start:
    ...
    def forward(self, xs, y, output_shape):
        # output_shape是xs[0]的shape[2:]
        assert isinstance(xs, tuple) or isinstance(xs, list)
        hs = []
        for i in range(len(xs)):
            # xs是包含多层U-Net左侧输出的列表
            # 遍历每个输入，传递给对应的CondRCUBlock进行计算
...
class ConvMeanPool(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size=3, biases=True, adjust_padding=False):
        ...
        if not adjust_padding:
            ...
            # adjust_padding增加0 padding，避免奇数行列进行均值池化
...
class ConditionalResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, num_classes, resample=None, act=nn.ELU(),
        ...
        self.input_dim = input_dim      # yaml中model.ngf
        self.output_dim = output_dim    # yaml中model.ngf
        self.resample = resample        # None
        if resample == 'down':
            if dilation is not None:
                self.conv1 = dilated_conv3x3(input_dim, input_dim, dilation=dilation)   # padding=dilation，使得尺寸不变。目的是让膨胀卷积扩大感受野，并没有下采样
                ...
                self.normalize2 = normalization(input_dim, num_classes)
                self.conv2 = ConvMeanPool(input_dim, output_dim, 3, adjust_padding=adjust_padding)  # 通过均值池化实现下采样
                conv_shortcut = partial(ConvMeanPool, kernel_size=1, adjust_padding=adjust_padding)

        elif resample is None:              # 不下采样
            if dilation is not None:
                conv_shortcut = partial(dilated_conv3x3, dilation=dilation)
    ...
    def forward(self, x, y):
        output = self.normalize1(x, y)      # 归一化
        output = self.non_linearity(output) # 激活
        output = self.conv1(output)         # 卷积
        output = self.normalize2(output, y) # 归一化
        output = self.non_linearity(output) # 激活
        output = self.conv2(output)         # 卷积
        # 恒等映射
...
class ConditionalInstanceNorm2dPlus(nn.Module):
    """根据噪声强度σ，调整归一化的仿射变换"""
    def __init__(self, num_features, num_classes, bias=True):   # num_classes 不是训练数据的类别数，而是噪声强度种类数
        super().__init__()
        self.num_features = num_features    # 通道数
        self.bias = bias
        self.instance_norm = nn.InstanceNorm2d(num_features, affine=False, track_running_stats=False)   # 禁止仿射变换
        if bias:
            self.embed = nn.Embedding(num_classes, num_features * 3)    # num_classes 不是训练数据的类别数，而是噪声强度种类数
            self.embed.weight.data[:, :2 * num_features].normal_(1, 0.02)  # Initialise scale at N(1, 0.02)：gamma, alpha
            self.embed.weight.data[:, 2 * num_features:].zero_()  # Initialise bias at 0: beta
        else:
            self.embed = nn.Embedding(num_classes, 2 * num_features)
            self.embed.weight.data.normal_(1, 0.02)

    def forward(self, x, y):
        means = torch.mean(x, dim=(2, 3))   # [B,C,H,W] 求出每个通道在H*W上的均值，符合IN
        m = torch.mean(means, dim=-1, keepdim=True) # 求出所有通道均值的均值
        v = torch.var(means, dim=-1, keepdim=True)  # 求出所有通道均值的方差
        means = (means - m) / (torch.sqrt(v + 1e-5))    # 标准化后的通道均值（可以理解为，与整体水平的差异）
        h = self.instance_norm(x)

        if self.bias:
            gamma, alpha, beta = self.embed(y).chunk(3, dim=-1) # 有点类似adaLN，获取gamma, alpha, beta
            h = h + means[..., None, None] * alpha[..., None, None] # alpha控制，标准化后的通道均值，需要添加多少
            out = gamma.view(-1, self.num_features, 1, 1) * h + beta.view(-1, self.num_features, 1, 1)  # gamma控制缩放，beta控制偏移
        else:
...
class CondRefineNetDilated(nn.Module):
    """实际上就是U-Net架构，有下采样的encoder，有上采样的decoder，并且decoder输入包含下层decoder输出以及同层encoder输出，与常规的U-Net最大差别就是decoder架构复杂并且非对称"""
    def __init__(self, config):
        super().__init__()
        self.logit_transform = config.data.logit_transform
        # self.norm = ConditionalInstanceNorm2d
        self.norm = ConditionalInstanceNorm2dPlus
        self.ngf = ngf = config.model.ngf   # 基础通道数
        self.num_classes = config.model.num_classes # 分类数，即sigma类别数
        self.act = act = nn.ELU()
        # self.act = act = nn.ReLU(True)

        self.begin_conv = nn.Conv2d(config.data.channels, ngf, 3, stride=1, padding=1)  # 开始卷积
        self.normalizer = self.norm(ngf, self.num_classes)

        self.end_conv = nn.Conv2d(ngf, config.data.channels, 3, stride=1, padding=1)    # 结束卷积
        ...
        self.res2 = nn.ModuleList([
            ConditionalResidualBlock(self.ngf, 2 * self.ngf, self.num_classes, resample='down', act=act,        # 下采样
                                     normalization=self.norm),
            ConditionalResidualBlock(2 * self.ngf, 2 * self.ngf, self.num_classes, resample=None, act=act,
                                     normalization=self.norm)]                  # 和下方代码中存在dilation的区别不大
        )

        self.res3 = nn.ModuleList([
            ConditionalResidualBlock(2 * self.ngf, 2 * self.ngf, self.num_classes, resample='down', act=act,    # 实际上并没有下采样，仍然维持尺寸，但扩大感受野
                                     normalization=self.norm, dilation=2),
            ConditionalResidualBlock(2 * self.ngf, 2 * self.ngf, self.num_classes, resample=None, act=act,
                                     normalization=self.norm, dilation=2)]      # 和上方代码中不存在dilation的区别不大
        )

        if config.data.image_size == 28:
            self.res4 = nn.ModuleList([
                ConditionalResidualBlock(2 * self.ngf, 2 * self.ngf, self.num_classes, resample='down', act=act,    # 扩大感受野
                                         normalization=self.norm, adjust_padding=True, dilation=4),
                ConditionalResidualBlock(2 * self.ngf, 2 * self.ngf, self.num_classes, resample=None, act=act,      # 扩大感受野
