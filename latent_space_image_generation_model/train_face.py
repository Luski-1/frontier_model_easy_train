...
class VAETrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._loss_history = {}                            # 存储最近一次 compute_loss 的拆分 loss
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        """
        # 1. 计算KL散度的权重
        kld_weight = self._get_kld_weight(model, self.state.global_step)
        model.config.kld_weight = kld_weight
    ...
    def prediction_step(self, model, inputs, prediction_loss_only=False, ignore_keys=None, **kwargs):
        ...
        # 可视化图像生成能力
        if self.save_eval_images and not self._eval_image_saved:
            self._eval_image_saved = True
            self._save_eval_images(model, inputs, outputs)
    ...
    def _save_eval_images(self, model, inputs, outputs):
        """
    ...
    def log(self, logs: dict, start_time=None):
        ...
        """
        # 在 Trainer 原始 log 之前，注入拆分 loss，后续自动写入tensorboadr

        logs['recon_loss'] = self._loss_history['recon']
        logs['kld_loss'] = self._loss_history['kld']
        logs['kld_weight'] = self._loss_history['kld_weight']

        # 自定义控制台打印
        print(f"[Step {self.state.global_step}] "
                f"recon={logs['recon_loss']:.2f} "
                f"kld={logs['kld_loss']:.4f} "
                f"weight={logs['kld_weight']:.4f}")
